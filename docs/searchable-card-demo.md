...
- **Gated and not run:** the single narrow paid Erling Haaland smoke test. It
  stays blocked until someone with account authority explicitly approves it;
  do not claim it ran in docs, demos, or the submission form.
- **Not captured yet:** browser/API live-mode recording of the searchable
  flow and the submission video.

## Test matrix

The branch was verified with the installed local toolchain: Prettier and
ESLint clean, six TypeScript projects clean, 24 test files / 333 tests passing,
six production builds passing, deterministic collector recovery passing, and
desktop/mobile browser checks clean. The `pnpm check` wrapper itself could not
start because pnpm attempted an unavailable registry metadata fetch and an
